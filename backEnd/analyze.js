...
const uniquePropertyCount = uniqueProperties.size;
...
const corruptListings = listings.filter(listing => {
    const carpetArea = Number(listing.carpet_area);
    const superBuiltUpArea = Number(listing.super_built_up_area);
    const floor = Number(listing.floor);
    const totalFloors = Number(listing.total_floors);
...
const assignedLocalityRentals = rentals.filter(rental => {
    return (
        typeof rental.locality === 'string' &&
        rental.locality.trim().toLowerCase() === ASSIGNED_LOCALITY
    );
});

const totalMonthlyRent = assignedLocalityRentals.reduce(
    (sum, rental) => {
...
const valid2BHKListings = listings.filter(listing => {
    const price = Number(listing.price);
    const carpetArea = Number(listing.carpet_area);
    ...
    return (
        ...
        !excludedListingIds.has(listing.listing_id) &&
...
const averagePricePerSqft =
    valid2BHKListings.length > 0
        ? totalPricePerSqft / valid2BHKListings.length
        : 0;

const avgPricePerSqft2BHK =
    Number(averagePricePerSqft.toFixed(2));
...
for (const project of projects) {
    ...
    if (
        ...
        priceMax > costliestProject.price_max_inr
    ) {
        costliestProject = {
...
const listingsLast7Days = recentListings.length;
...
for (const listing of listings) {
    ...
    actualCounts[listing.project_id] =
        (actualCounts[listing.project_id] || 0) + 1;
}
...
const answers = {
    ...
    unique_properties: uniquePropertyCount,
